...
function Avatar({ name, size = "md", className = "" }) {
  ...
  return (
    <div
      className={`${sz} rounded-full bg-gradient-to-br from-cyan-500 to-purple-600 flex items-center justify-center text-white font-bold shrink-0 ${className}`}
    >
      {initials(name)}
    </div>
  );
...
function OverviewTab({ navigate, setSearchParams }) {
  ...
  const stats = [
    ...
    {
      label: "Reports Ready",
      value: DIAGNOSES.filter((d) => d.report).length,
      Icon: FileText,
      color: "text-blue-500",
      bg: "bg-blue-500/10",
    },
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 p-7 mb-7 text-white">
        ...
        <p className="text-xs uppercase tracking-widest text-cyan-100 mb-1">
          Patient Dashboard
        </p>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => goTab("appointments")}
              className="text-xs text-cyan-500 hover:text-cyan-600 font-semibold flex items-center gap-1"
            >
          ...
          <div className="space-y-3">
            {DIAGNOSES.slice(0, 4).map((d) => (
              ...
              >
                ...
                <span
                  className={`text-[11px] font-bold px-2 py-0.5 rounded-full shrink-0 ml-2 ${d.report ? "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400" : "bg-slate-100 dark:bg-slate-800 text-slate-400"}`}
                >
        ...
        {DIAGNOSES.map((d, i) => (
          ...
          >
            <Card hover className="p-4 sm:p-5">
              ...
              <div className="sm:hidden mt-2 text-[11px] text-slate-400">
                {fmtDate(d.date)}
              </div>
            </Card>
          </motion.div>
...
export default function PatientDashboard() {
  ...
  const CONTENT = {
    overview: (
      <OverviewTab navigate={navigate} setSearchParams={setSearchParams} />
    ),
