...
const APPOINTMENTS = [
  ...
  },
  {
    ...
    serial: 5,
    status: "Completed",
    note: "Routine cardiac screening",
  },
  {
    id: 6,
...
const PATIENTS = [
  ...
  },
  {
    ...
    city: "Khulna",
    visits: 1,
    last_visit: "2025-03-29",
    condition: "CAD workup",
  },
];
...
function Avatar({ name, size = "md", className = "" }) {
  ...
  return (
    <div
      className={`${sz} rounded-full bg-gradient-to-br from-teal-500 to-blue-600 flex items-center justify-center text-white font-bold shrink-0 ${className}`}
    >
      {initials(name)}
    </div>
  );
...
function OverviewTab({ setSearchParams }) {
  ...
  const todayLeft = todayAppts.filter(
    (a) => a.status === "Pending" || a.status === "Confirmed",
  ).length;
  const nextPatient = todayAppts.find(
  ...
  const stats = [
    {
      ...
      color: "text-teal-500",
      bg: "bg-teal-500/10",
      sub: `${todayDone} done · ${todayLeft} remaining`,
    },
    {
      label: "Total Patients",
      value: DOCTOR.total_patients,
      Icon: Users,
      color: "text-blue-500",
      bg: "bg-blue-500/10",
      sub: "All time",
  ...
  const recent = APPOINTMENTS.filter((a) => a.status === "Completed").slice(
    0,
    3,
  );
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-teal-500 via-blue-600 to-blue-700 p-7 mb-7 text-white">
        ...
        <div className="flex flex-wrap items-center gap-5">
          ...
          />
          <div>
            <p className="text-xs uppercase tracking-widest text-teal-100 mb-1">
              Doctor Portal
            </p>
            <h2 className="text-2xl font-bold mb-0.5">{DOCTOR.doctor_name}</h2>
            <p className="text-sm text-blue-100">
              {DOCTOR.specialization} · {DOCTOR.branch}
            </p>
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              <span className="inline-flex items-center gap-1.5 bg-white/15 border border-white/25 text-white text-xs font-semibold px-3 py-1 rounded-full">
                <Star className="w-3.5 h-3.5 fill-white" /> {DOCTOR.rating}{" "}
                Rating
              </span>
            </div>
          </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              className="text-xs text-teal-500 hover:text-teal-600 font-semibold flex items-center gap-1"
            >
          ...
          <div className="mb-4">
            ...
            <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: todayAppts.length
                    ? `${(todayDone / todayAppts.length) * 100}%`
                    : "0%",
                }}
              />
            </div>
          ...
          {nextPatient ? (
            <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800/40 rounded-xl p-3.5">
              <p className="text-[10px] uppercase tracking-widest text-teal-500 font-bold mb-2">
                Next Patient
              </p>
              <div className="flex items-center gap-3">
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-bold text-teal-600 dark:text-teal-400">
                    {fmtTime(nextPatient.time)}
                  </p>
                  <p className="text-[10px] text-slate-400">
                    Serial #{nextPatient.serial}
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Remaining list */}
          {todayAppts
            .filter((a) => a.status !== "Completed" && a.id !== nextPatient?.id)
            .slice(0, 3)
            .map((a) => (
              <div
                key={a.id}
                className="flex items-center gap-3 py-2.5 border-t border-slate-100 dark:border-slate-800 mt-2"
              >
                <PatientAvatar name={a.patient} size="sm" />
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex-1 truncate">
                  {a.patient}
                </p>
        ...
        <Card className="p-5">
          ...
          <div className="space-y-0">
            {recent.map((a, i) => (
              <div
                key={a.id}
                className={`flex items-center gap-3 py-3 ${i < recent.length - 1 ? "border-b border-slate-100 dark:border-slate-800" : ""}`}
              >
...
function AppointmentsTab() {
  ...
  const filtered =
    filter === "All"
      ? APPOINTMENTS
      : APPOINTMENTS.filter((a) => statuses[a.id] === filter);
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] text-slate-400 dark:text-slate-600 mt-4 text-center">
        Tap <ChevronRight className="inline w-3 h-3" /> on any row to cycle its
        status
      </p>
    </motion.div>
  );
              ...
              {s.active ? (
                <>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Array.from({ length: Math.min(s.slots, 14) }).map(
                      (_, j) => (
                        <span
                          key={j}
                          className="w-2 h-2 rounded-full bg-teal-400/60 dark:bg-teal-500/40"
                        />
                      ),
                    )}
                    {s.slots > 14 && (
...
function ProfileTab() {
  const fields = [
    ...
    {
      label: "Joined",
      value: fmtDate(DOCTOR.joining_date),
      Icon: CalendarDays,
    },
